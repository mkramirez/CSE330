...
int main() {
    ...
    for (int i = 1; i <= 6; i++) {
        ...
        Random::random_vector(how_many, 1, 50, mynums);

        int no_ops = 0;
        int maxsum1 = mss::max_subseq_sum_cube(mynums, no_ops);
        std::cout << "max_....cube: " << maxsum1 << " " << no_ops << std::endl;

        int maxsum2 = mss::max_subseq_sum_quad(mynums, no_ops);
        std::cout << "max_....quad: " << maxsum2 << " " << no_ops << std::endl;

        int maxsum3 = mss::max_subseq_sum_lin(mynums, no_ops);
